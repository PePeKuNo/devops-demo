...
const JOBS_QUEUE_KEY = 'jobs:item-created';
...
function createApp(options = {}) {
  ...
  app.use((req, res, next) => {
    requestCount += 1;
    res.set('X-Backend-Instance', backendInstanceId);
    next();
  });
  ...
  app.post('/items', asyncHandler(async (req, res) => {
    ...
    try {
      if (typeof cache.enqueue === 'function') {
        await cache.enqueue(JOBS_QUEUE_KEY, JSON.stringify({
          type: 'item-created',
          itemId: newProduct.id,
          itemName: newProduct.name,
          createdBy: backendInstanceId,
          createdAt: new Date(now()).toISOString()
        }));
      }
    } catch (error) {
      console.error(`Nie udalo sie dodac zadania do kolejki: ${error.message}`);
    }
